...
const CheckReservation = () => {
  ...
  const handleCancelReservation = async () => {
    ...
    setIsLoading(true);
    try {
      const response = await fetch(`https://little-lemon-backend-j1dg.onrender.com/api/reservations/${reservationToCancel._id}`, {
        method: 'DELETE'
      });
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const response = await fetch(`https://little-lemon-backend-j1dg.onrender.com/api/reservations?${query}`);
      const data = await response.json();
